...
const TTSPage = ({ config, onNotification, activeSubPage, onSubPageChange }) => {
  ...
  const [asrResultType, setAsrResultType] = useState('success')
  ...
  // 初始化加载角色
  useEffect(() => {
    if (activeSubPage === 'voice-synthesis' && currentMode === 'character' && voiceCharacters.length === 0) {
      loadVoiceCharacters()
    }
  }, [currentMode, config.ttsApiUrl, activeSubPage])
  ...
  const handleTranscribe = async () => {
    ...
    try {
      ...
      setAsrResult({ error: error.message })
      setAsrResultType('error')
      setShowAsrResult(true)
  ...
  const handleDownload = () => {
    if (!asrResult?.content) return

    const content = typeof asrResult.content === 'string' 
      ? asrResult.content 
      : JSON.stringify(asrResult.content, null, 2)

    const blob = new Blob([content], { 
      type: asrResult.format === 'json' ? 'application/json' : 'text/plain' 
    })
    
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = asrResult.filename.replace(/\.[^/.]+$/, '') + 
                 (asrResult.format === 'json' ? '_transcription.json' : '_transcription.txt')
    a.click()
  ...
  const asrResultActions = asrResultType === 'success' ? [
    {
      label: '📥 下载文本',
      variant: 'primary',
    ...
    <div className="tts-page max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-8 pb-4 border-b border-gray-200">
        ...
        <h2 className="text-2xl font-semibold text-gray-900">
          {activeSubPage === 'voice-synthesis' ? '语音合成' : '语音识别'}
        </h2>
      </div>
      ...
      {activeSubPage === 'speech-recognition' && (
        <div className="space-y-6">
          ...
          {/* 结果展示 */}
          <ResultPanel
            isVisible={showAsrResult}
            type={asrResultType}
            title={asrResultType === 'success' ? '语音识别完成' : '语音识别失败'}
            actions={asrResultActions}
          >
            {asrResultType === 'success' ? (
              <div>
                <div className="bg-gray-50 p-4 rounded-lg mb-3 border border-gray-200">
                  <pre className="whitespace-pre-wrap font-inherit text-sm text-gray-800 m-0">
                    {typeof asrResult?.content === 'string' 
                      ? asrResult.content 
                      : JSON.stringify(asrResult?.content, null, 2)}
                  </pre>
                </div>
              ...
              <p className="text-red-700">错误信息: {asrResult?.error}</p>
            )}
          </ResultPanel>
        </div>
      )}
